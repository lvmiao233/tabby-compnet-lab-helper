...
│                                         │
│ ──────────────────────────────────────  │
│ 由 Tabby Compnet Lab Helper 插件生成    │
└─────────────────────────────────────────┘
```
...
```
🖼️ Tabby Compnet Lab Helper图片导出开始     [绿色背景]
📊 准备导出 2 个命令区块
🎨 开始将HTML渲染为图片...    [橙色背景]
📐 图片尺寸: 800x600          [紫色背景]
...
```
控制台输出:
🚀 Tabby Compnet Lab Helper插件已加载 - 终端底部状态栏模式
📸 CaptureService 初始化
🎯 进入捕获模式
🔍 开始获取当前终端实例...
...
4. **过滤Tabby Compnet Lab Helper日志**：
   - 在控制台中输入: `Tabby Compnet Lab Helper`
   - 或搜索关键词: `🖼️` `🎨` `💾` 等表情符号
...
**Tabby Compnet Lab Helper v0.2.16** - 主题同步和UI优化版本
...
- 🎨 **图标按钮优化**：使用SVG图标，添加边框，提高对比度和视觉效果
- 🎨 **智能下载**：优先使用File System Access API，支持目录记忆和自动保存
- 🎨 **渐进式降级**：API不支持时自动降级到传统下载方式
- 🎨 **设置面板**：提供插件设置界面，支持下载目录配置
- 🎨 **并发控制**：防止多个文件选择器同时打开，避免"File picker already active"错误
- 🎨 **智能退出**：文件保存完成后自动退出捕获模式，提升用户体验
- 🎨 **优化图片内容**：移除标题和时间戳，只保留命令内容，减少空白，自适应高度
- 🎨 **清理图片页脚**：移除插件生成标识，保持图片简洁
- 🎨 **精准行数控制**：移除自动换行，保持与终端显示完全一致的行数
- 🎨 **自适应图片宽度**：根据内容最长行自动调整图片宽度
- 🎨 **全屏模态框**：命令选择窗口尺寸优化为95%视口大小，提供更大空间
- 🎨 **移除状态条**：清理历史遗留功能，界面更加简洁
- 🎨 **移除快捷键**：不再需要复杂的键盘快捷键操作
- 🎨 **优化布局**：使用flex布局消除底部空白，提供完美填充
- 🎨 **智能关闭**：关闭窗口时自动退出捕获模式，提升用户体验
...
### 基础使用
1. **启动插件**：重启Tabby后，工具栏会出现照相机按钮 📷
2. **进入捕获模式**：点击工具栏按钮进入捕获模式
3. **选择命令区块**：在弹出的窗口中选择要导出的命令区块
4. **导出截图**：点击"复制到剪贴板"或"下载并复制"按钮
5. **查看结果**：图片会自动保存并复制到剪贴板
...
- 每次使用都需要用户确认选择目录（浏览器的安全机制）
- 选择后可以直接点击确认，无需重新浏览目录
- 文件写入完成后会自动打开文件所在文件夹
- 支持：Chrome 86+, Edge 86+, 其他现代浏览器

**传统下载（后备）**：
- 当浏览器不支持File System API时自动启用
- 使用浏览器原生下载功能
- 每次下载都会弹出保存对话框
...
插件提供设置面板用于配置下载选项：
- 在Tabby设置中找到"Netty 插件"选项卡
- 可以设置默认下载目录
- 查看API支持状态
- 获取详细的使用说明

**注意**：
- 首次使用File System API时，浏览器会请求文件系统访问权限。请允许此权限以获得最佳体验。
- 文件保存完成后，插件会自动退出捕获模式，返回正常状态。
- 如果遇到"File picker already active"错误，请关闭其他文件对话框后重试。
